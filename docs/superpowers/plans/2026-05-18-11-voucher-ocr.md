...
```bash
docker compose exec postgres psql -U bookmee -d bookmee -c '\d "Voucher"' | grep ocrError
```
...
```bash
cd /home/kkouta/poc/bookmee/server && npx tsc --noEmit 2>&1 | grep -v "loggerInstance\|Http2\|routes/mode" | head -20
```
