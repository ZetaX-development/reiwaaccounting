...
- 牧野氏: 「マネフォはフリーのデータを見に行けないわけだから、そこがzeimeeの取れるところ」
- 08 との合わせ技: zeimee は OCR / 自動仕訳の生データ表示は freee/MF に任せ、**横串の集約レイヤー**に集中する。これがプロダクトの差別化の核。
...
5. zeimee は freee/MF の生取引画面を**そのまま再現しない**（重複機能の排除）
